...
function getPanes() {
    const panesFilter = (panes, prop, name) => panes.filter(x => x[prop]
                     && x[prop].constructor && x[prop].constructor.name === name);
    let allPanes = atom.workspace.getPanes(),
        panes = panesFilter(allPanes, 'parent', 'PaneAxis');
    if (!panes.length) panes = panesFilter(allPanes, 'activeItem', 'TextEditor');
    return panes;
}
